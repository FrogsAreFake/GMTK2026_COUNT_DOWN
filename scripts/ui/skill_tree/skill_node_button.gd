...
func _gui_input(event: InputEvent) -> void:
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT:
		...
		# While the mouse stays over this button, it (not SkillTreeView) receives motion
		# events, so the drag threshold and panning must be handled here too.
		SkillTreePanState.update_press(get_global_mouse_position())
		if SkillTreePanState.dragging and SkillTreePanState.moved_beyond_threshold:
